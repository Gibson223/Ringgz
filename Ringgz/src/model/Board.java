...
public class Board {
    ...
    /**
     * The DIM by DIM fields of the Ringgz student. See NUMBERING for the coding of the fields.
     */
    ...
    public boolean adjacentHasBase(int field, Ring ring) {
        if (ring.getTier() == Tier.BASE) {
            for (Field fieldforbase : this.adjacentFields(field)) {
                if (fieldforbase.hasBase() && (fieldforbase.getFieldState().stream()
                        .anyMatch(a -> a.getColor() == ring.getColor()))) {
                    return true;
                }
            }
        }
     ...
     * @return true if there is a ring with that color in the adjacent fields of the specified
     *         field.
     */
