...
public class HumanPlayer extends Player {
	 ...
	 */
	/**
	 * Asks the user to input the field where to place the next ring, as well as
	 * what size of ring. This is done using the standard input/output.
	 * 
	 * @param board
	 *            the game board
	...
	public Move makeMove(Board board) {
		...
		String promptRing = "> " + getName() + " (" + getPrimaryColor().toString() + ")"
				+ ", what kind of ring will you place?" + ""
				+ "(1(BASE),2(SMALLEST),3(SMALL),4(MEDIUM),5(LARGE))";
		String promptColor = "> " + getName() + " (" + getPrimaryColor().toString() + ")"
				+ (this.getSecondaryColor() == null ? "" : "or secondary color +" + this.getSecondaryColor().toString())
